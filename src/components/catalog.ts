...
export interface ComponentDoc {
  name: string;
  category: 'shell' | 'prose' | 'interactive' | 'visual' | 'media';
  whenToUse: string;
  whenNotToUse?: string;
  props: PropDoc[];
  example: string;
...
export const COMPONENTS: ComponentDoc[] = [
  {
    ...
    category: 'shell',
    whenToUse:
      'ALWAYS the outermost component of a lesson. Provides the page frame, back link, title block, and consistent typography.',
    props: [
...
>
  ...
  },
  {
    ...
    category: 'shell',
    whenToUse:
      'Top-level chapter of a lesson. Each Section becomes its own PAGE in the paginated lesson shell — design it to stand alone on a single screen. Use 3–7 Sections per lesson.',
    props: [
  ...
  },
  {
    ...
    category: 'prose',
    whenToUse:
      'Highlight a key insight, an aside, a warning, or a quick info box. Use sparingly — at most 1–2 per section.',
    props: [
  ...
  },
  {
    ...
    whenToUse:
      'Multiple-choice check-in. Use at section boundaries to reinforce a concept just taught. Aim for 1 per major section.',
    whenNotToUse:
      'Avoid for trivia. Quiz questions should test understanding, not recall of a specific number/word.',
    props: [
...
/>`,
  },
  {
    ...
    category: 'interactive',
    whenToUse:
      'Get the learner to articulate an answer in their own words before revealing a sample. Use for "explain why" or "predict what happens" prompts.',
    props: [
...
/>`,
  },
  {
    ...
    category: 'interactive',
    whenToUse:
      'Hide an answer, hint, or extra detail until the learner clicks. Great after posing a question inline in prose.',
    props: [
...
<Reveal label="Show answer">
  ...
  },
  {
    ...
    category: 'interactive',
    whenToUse:
      'Let the learner manipulate a parameter and see the effect. Pair with a Plot, Math, or Sandbox to drive a simulation.',
    props: [
...
return (
  ...
  },
  {
    ...
    category: 'prose',
    whenToUse:
      'Inline definition for a technical term — hover/tap reveals the definition. Use the FIRST time a term appears.',
    props: [
  ...
  },
  {
    ...
    category: 'visual',
    whenToUse:
      'Render a Mermaid diagram — flowcharts, sequence diagrams, state machines, simple graphs.',
    whenNotToUse:
      'Not for charts of numerical data — use <Plot>. Not for math equations — use <Math>.',
    props: [
...
/>`,
    notes:
      'Mermaid is finicky about syntax — keep diagrams simple. Always test with a single arrow before adding complexity.',
  },
  {
    name: 'Math',
    category: 'visual',
    whenToUse:
      'Render LaTeX math. Use inline (default) for in-sentence formulas; use display=true for centered, larger equations.',
    props: [
...
<Math display expr="\\\\frac{d}{dt}\\\\left[ \\\\sum_i m_i v_i \\\\right] = F_{ext}" />`,
  },
  {
    ...
    category: 'visual',
    whenToUse:
      'Line or bar chart of numerical data. Use when the relationship between numbers is itself the lesson.',
    props: [
...
/>`,
  },
  {
    ...
    category: 'interactive',
    whenToUse:
      'Container for a custom interactive simulation. Put a visual/output in children, and put input widgets (Slider, buttons) in the controls slot.',
    props: [
  ...
  return (
    ...
    notes:
      'A Sandbox is a function-component pattern: define a small inner component that owns useState, render the Sandbox from it. The lesson default export wires these together.',
  },
  {
    name: 'StepThrough',
    category: 'interactive',
    whenToUse:
      'Walk the learner through a sequence of states one at a time (e.g., steps of a derivation, frames of an animation, stages of a process).',
    props: [
  ...
  },
  {
    ...
    category: 'interactive',
    whenToUse:
      'Drill on a set of term↔definition pairings. Best for vocabulary or concept-association practice.',
    props: [
  ...
  },
  {
    ...
    category: 'media',
    whenToUse:
      'Drop a video placeholder where a generated clip belongs. For v1 it renders a styled placeholder; once video generation is wired up, pass src to render the actual video.',
    props: [
...
export function catalogIndexText(): string {
  ...
  lines.push('');
  lines.push(
    'All from `@/components`. Names only here — call `describe_components(["Name", ...])` for full props + examples on the components you actually use.',
  );
  lines.push('');
...
function describeOne(c: ComponentDoc): string {
  const lines: string[] = [];
  lines.push(`## <${c.name}>  · ${c.category}`);
  lines.push('');
  lines.push(`**Use for:** ${c.whenToUse}`);
  if (c.whenNotToUse) lines.push(`**Avoid for:** ${c.whenNotToUse}`);
  lines.push('');
  lines.push('**Props:**');
...
export function describeComponentsText(names: string[]): string {
  if (names.length === 0) return 'No component names provided.';
  const known = new Map(COMPONENTS.map((c) => [c.name, c]));
  const sections: string[] = [];
  const unknown: string[] = [];
  for (const n of names) {
    const c = known.get(n);
    if (c) sections.push(describeOne(c));
    else unknown.push(n);
  }
