...
import { PROD, MANIFEST } from './productions/looping-llms.generated';
...
function shouldRenderMovie() {
  if (typeof window === 'undefined') return false;
  const params = new URLSearchParams(window.location.search);
  const movie = params.get('movie');
  return movie === '1' || movie === 'true';
}

export default function LoopingLLMs() {
  const [renderMovie] = useState(shouldRenderMovie);

  if (renderMovie) {
    return (
      <main className="min-h-screen bg-black p-4">
        <div className="mx-auto h-[calc(100vh-2rem)] max-w-7xl">
          <Playback
            production={PROD}
            manifest={MANIFEST}
